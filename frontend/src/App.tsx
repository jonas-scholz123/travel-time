...
function getCoords(searchParams: URLSearchParams): [number, number][] {
  return searchParams.getAll("coords").map(c => c.split(",")).map(c => [parseFloat(c[0]), parseFloat(c[1])]);
}
...
function App() {
  ...
  const [coordsList, setCoordsList] = useState(getCoords(searchParams));
  ...
  const addLoc = (location: string, coord: [number, number]) => {
  ...
  return (
    <div className="h-screen">
      <TravelTimeMap addLoc={addLoc} changeView={changeView} coordsList={coordsList} CircleLayer={<CircleLayer circles={circles} />} />
